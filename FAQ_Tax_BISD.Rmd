...
# Have to add in 23 & 24 tax rates from different file cause public data stinks..

tmp <- levies_2023[,.(`School District`=toupper(`Taxing Unit Name`[1]),
					  `2023-2024 M&O Tax Rate`=`M & O Rate`[1],
					  `2023-2024 I&S Tax Rate`=`I & S Rate`[1]),by=`Taxing Unit Name`]
setkey(tmp, `School District`)
setkey(tax_rates,`School District`)
tax_rates <- tax_rates[tmp[,.(`School District`,`2023-2024 M&O Tax Rate`,`2023-2024 I&S Tax Rate`)]]
...
# 																		 `Combined Tax Rate`=levies_2024[`TAXING UNIT NAME`=="Boerne ISD"]$`TOTAL TAX RATE`[1]))
...
# BISD_neighbors_combined_tax_rate <- rbind(BISD_neighbors_combined_tax_rate,
#                                           neighbor_districts[,.(`School District` = Name,
#                                                                 `Tax Year` = "2024",
#                                                                 `Combined Tax Rate` = levies_2024[toupper(`TAXING UNIT NAME`) == Name & toupper(`COUNTY NAME`) == County]$`TOTAL TAX RATE`[1]),
#                                                              by=CDN][,.(`School District`,`Tax Year`,`Combined Tax Rate`)])
BISD_neighbors_combined_tax_rate <- rbind(BISD_neighbors_combined_tax_rate[`School District` != "BOERNE ISD"],
                                          BISD_neighbors_combined_tax_rate[`School District` == "BOERNE ISD"])


BISD_neighbors_combined_tax_rate[,Rank:=rank(-`Combined Tax Rate`),by=`Tax Year`]
...
per_student_funding <- finance_summary[`DISTRICT NAME`=="BOERNE ISD",.SD,.SDcols=cols]
...
# kcad_values[year==2016,tax_rate:=BISD_combined_tax_rate[`Tax Year`==2016]$`Combined Tax Rate`]
# kcad_values[year==2024,tax_rate:=BISD_combined_tax_rate[`Tax Year`==2023]$`Combined Tax Rate`]
...
tags_source <- list(BISD_tax = character())


# Cost of Recapture Since 2007
tags_source[["BISD"]] <- c(tags_source[["BISD"]],
						   `{{{TOTAL_RECAPTURE_2008_2024}}}` = finance_summary[`DISTRICT NAME`=="BOERNE ISD",
...
# Levied Taxes
tags_source[["BISD"]] <- c(tags_source[["BISD"]],
                               `{{{2024_COMBINED_RATE}}}` = levies_2024[`TAXING UNIT NAME`=="Boerne ISD",`M&O RATE` + `I&S RATE`][1])
tags_source[["BISD"]] <- c(tags_source[["BISD"]],
                               `{{{2024_LEVY}}}` = scales::label_comma()(levies_2024[`TAXING UNIT NAME`=="Boerne ISD",sum(`CALCULATED LEVY`)]))
tags_source[["BISD"]] <- c(tags_source[["BISD"]],
                               `{{{2024_MARKET_VALUE}}}` = scales::label_comma()(levies_2024[`TAXING UNIT NAME`=="Boerne ISD",sum(`MARKET VALUE`)]))

# Tax Rate
tags_source[["BISD"]] <- c(tags_source[["BISD"]],
                               `{{{2005_TAX_RATE}}}` = BISD_combined_tax_rate[`Tax Year`==2005]$`Combined Tax Rate`)
tags_source[["BISD"]] <- c(tags_source[["BISD"]],
                               `{{{2024_TAX_RATE}}}` = BISD_combined_tax_rate[`Tax Year`==2024]$`Combined Tax Rate`)
tags_source[["BISD"]] <- c(tags_source[["BISD"]],
                               `{{{MEDIAN_TAX_RANK}}}` = BISD_neighbors_combined_tax_rate[`School District`=="BOERNE ISD", median(Rank)])
tags_source[["BISD"]] <- c(tags_source[["BISD"]],
                               `{{{2024_TAX_RANK}}}` = BISD_neighbors_combined_tax_rate[`School District`=="BOERNE ISD" & `Tax Year` == 2024]$Rank)


# Enrollment
tags_source[["BISD"]] <- c(tags_source[["BISD"]],
                               `{{{2024_LEVY_PER_STUDENT}}}` = scales::label_comma()(round(round(levies_2024[`TAXING UNIT NAME`=="Boerne ISD",sum(`CALCULATED LEVY`)]/directory[District.Name=="BOERNE ISD"]$`District.Enrollment.as.of.Oct.2024`[1]))))
...
tags_source[["BISD"]] <- c(tags_source[["BISD"]],
                               `{{{2008_REVENUE_PER_STUDENT}}}` = scales::dollar_format()(tmp[factor_year==2008]$funding))
tags_source[["BISD"]] <- c(tags_source[["BISD"]],
                               `{{{2024_REVENUE_PER_STUDENT}}}` = scales::dollar_format()(tmp[factor_year==2024]$funding))
...
tables[[6]] <- pdata
figures[[6]] <- plot_ly(pdata, x = ~YEAR, y = ~`Recapture Payment`, 
											  type = 'scatter', name="Recapture", mode = 'lines+markers',
												title = "BISD Recapture Since 2007",
												height=200)  %>%
  layout(title = "BISD Recapture Since Class Year 2008", 
  			 xaxis=list(title="Class Year"),
...
tables[[3]] <- copy(BISD_combined_tax_rate)
figures[[3]] <- plot_ly(BISD_combined_tax_rate, x = ~`Tax Year`, y = ~`Combined Tax Rate`, 
											  type = 'scatter', name="Combined Tax Rate", mode = 'lines+markers',
												height=200)  %>%
  layout(title = "BISD Combined M&O and I&S Tax Rate 2005-2023", 
...
pdata <- copy(BISD_neighbors_combined_tax_rate)
...
fig <- plot_ly(pdata,
  ...
  layout(title = "BISD and Neighboring District's Tax Rates 2005-2023", 
         xaxis = list(title = "Year"),
         yaxis = list(title = "Combined Tax Rate", 
                      range = c(.75,2), 
                      tickvals = list(.75,1,1.25,1.5,1.75,2),
...
pdata[,per_student_with_recap:=`GEN FUNDS-TOTAL OPERATING REVENUE AND OTHER REVENUE AND RECAPTUR`/`FALL SURVEY ENROLLMENT`*factor]
pdata[,per_student_no_recap:=`GEN FUNDS-TOTAL OPERATING REVENUE AND OTHER REVENUE`/`FALL SURVEY ENROLLMENT`*factor]
figures[[5]] <- plot_ly(pdata, x = ~factor_year, y = ~per_student_with_recap, 
                        type = 'scatter', name="Including\nRecapture", mode = 'lines+markers',
                        height=200)  %>%
  add_trace(y=~per_student_no_recap, 
            type = 'scatter', mode='lines+markers', name='Excluding\nRecapture') %>%
  layout(title = "BISD Revenue Per Student (General Funds) 2007-2023", 
         xaxis=list(title="Year"),
